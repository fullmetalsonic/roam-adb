...
- 계획 승인: 2026-08-25
- 현재 단계: 부품 구매·배송 대기, 구현 전
- 기준 휴대폰: Samsung Galaxy Z Fold8, Android 17, One UI 9
- 1차 브리지 후보: Raspberry Pi Zero 2 W
- 공개 범위: 공개 가능한 설계·합성 시험만 GitHub Public에 기록
- 구현 경계: 부품 도착 전 OS 기록, 설치 스크립트, Bridge Agent, APK와 Release를 만들지 않는다.

이 문서는 현재 공개된 `v0.1.2-spike` 무선 ADB 경로를 대체하거나 해당 릴리스가 USB 브리지를 지원한다고 주장하지 않는다. 새 경로의 모드 식별자는 계획상 `portable-usb-bridge`이며 실기기 스파이크를 통과하기 전까지 **계획·미구현**으로 표시한다.
...
- Fold8 자체 핫스팟이 휴대형 브리지에 인터넷을 제공한다.
...
│                                                        │
│ Wi-Fi client ── Fold8 2.4 GHz hotspot ── LTE/5G        │
│ Tailscale node                                          │
...
| 구성요소 | 역할 | 저장하는 신뢰 정보 |
|---|---|---|
| Fold8 | USB `adbd`, LTE/5G, 핫스팟 | 브리지 ADB RSA 공개키 승인 |
...
Fold8의 Tailscale 앱은 이 경로의 데이터 운반에 필수가 아니다. 브리지가 Fold8 핫스팟을 일반 인터넷 회선으로 사용해 자기 Tailscale node를 연결한다. 폰의 Tailscale은 다른 용도가 있을 때 독립적으로 켤 수 있지만 1차 브리지 E2E에서는 의존하지 않는다.
...
- 결과: LTE/5G 핫스팟을 통한 집 PC→브리지→USB ADB 왕복이 실제로 성공해야 한다.

### H-07 자동화와 복구

- 전원 인가 뒤 Wi-Fi, Tailscale과 ADB 감시를 순서대로 시작한다.
- 핫스팟 OFF/ON, LTE 전환, USB 분리·재연결과 브리지 재부팅을 시험한다.
...
| `HOTSPOT_REQUIRED` | 알려진 2.4GHz 망 없음 | Fold8 핫스팟 켜기 |
...
| `RECOVERING` | 핫스팟·USB·Tailscale 복구 중 | 대기 또는 중지 |
...
| 핫스팟 자동 종료 | 원격 세션 끊김 | 절전 설정 확인, 제한된 재접속 |
...
|---|---|
| 아키텍처 타당성 | 문서 검토 PASS |
| Zero 2 W USB OTG·2.4GHz | 공식 사양 확인 |
| ARM용 ADB 패키지 | Debian 패키지 확인 |
| Fold8 핫스팟 + Pi Tailscale | HARDWARE REQUIRED |
| Fold8 USB ADB + Pi | HARDWARE REQUIRED |
| 집 PC 원격 ADB E2E | HARDWARE REQUIRED |
| 코드·설치 스크립트·Bridge Agent | NOT IMPLEMENTED |
| 새 APK·EXE·Release | NOT CREATED |

부품이 도착하면 H-00부터 시작한다. 사용자가 처음 Raspberry Pi를 설정한다는 전제로, 한 단계의 실제 화면과 결과를 확인하기 전에는 다음 단계로 넘어가지 않는다.
